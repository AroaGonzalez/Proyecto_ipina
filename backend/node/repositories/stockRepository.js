  ...
  try {
    ...
    let searchQuery = `
      ...
      FROM AJENOS.STOCK_LOCALIZACION_RAM slr
        ...
        INNER JOIN MAESTROS.ESTADO_TIENDA et ON et.ID_ESTADO_TIENDA = th.ID_ESTADO_TIENDA
        INNER JOIN MAESTROS.ESTADO_TIENDA_IDIOMA eti ON eti.ID_ESTADO_TIENDA = et.ID_ESTADO_TIENDA AND eti.ID_IDIOMA = :idIdioma
    ...
    let countQuery = `
      SELECT COUNT(DISTINCT CONCAT(slr.ID_ALIAS, '_', slr.ID_LOCALIZACION_COMPRA))
        FROM AJENOS.STOCK_LOCALIZACION_RAM slr
        INNER JOIN AJENOS.ALIAS a ON a.ID_ALIAS = slr.ID_ALIAS
        INNER JOIN AJENOS.TIPO_ESTADO_ALIAS_IDIOMA teai ON teai.ID_TIPO_ESTADO_ALIAS = a.ID_TIPO_ESTADO_ALIAS
        AND teai.ID_IDIOMA = :idIdioma
