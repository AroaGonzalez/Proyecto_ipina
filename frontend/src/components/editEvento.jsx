...
const EditEvento = () => {
  ...
  useEffect(() => {
    const fetchEvento = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${BASE_URL}/eventos/${id}?idIdioma=${languageId}`);
        ...
        if (eventoData.tareas && eventoData.tareas.length > 0) {
            ...
            setSelectedTipoTarea(tipoTareaFromTareas || null);
        }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const eventoData = {
        ...
        idTipoEvento: evento.idTipoEvento || 2,
        idTipoEstadoEvento: evento.idTipoEstadoEvento || 2,
      ...
      await axios.put(`${BASE_URL}/eventos/${id}`, eventoData);
      navigate('/eventos');
